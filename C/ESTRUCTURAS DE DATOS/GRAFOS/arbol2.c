...
// FUNCION ELIMINAR NODO AGREGADA
int eliminarNodo(struct nodo **ptrRef, int dato){
    ...
    if(dato < (*ptrRef)->dato)
        return eliminarNodo(&((*ptrRef)->pI), dato);
    else if(dato > (*ptrRef)->dato)
        return eliminarNodo(&((*ptrRef)->pD), dato);
    else {
        nodoBasura = *ptrRef;
        
        if((*ptrRef)->pI == NULL && (*ptrRef)->pD == NULL) {
            free(*ptrRef);
            *ptrRef = NULL;
        }
        else if((*ptrRef)->pI == NULL) {
            *ptrRef = (*ptrRef)->pD;
            free(nodoBasura);
        }
        else if((*ptrRef)->pD == NULL) {
            *ptrRef = (*ptrRef)->pI;
            free(nodoBasura);
        }
        else {
            struct nodo *ptrAva = (*ptrRef)->pI;
            struct nodo *ptrAtr = *ptrRef;
            
            while(ptrAva->pD != NULL) {
                ptrAtr = ptrAva;
                ptrAva = ptrAva->pD;
            }
...
int menu(){
    ...
    printf("\n7. Eliminar nodo de Arbol 1");
    printf("\n8. Eliminar nodo de Arbol 2");
    printf("\n9. Salir");
    printf("\nOpcion: ");
...
int main(){
    ...
    for(;;){
        ...
        case 7:
            printf("\nNodo a eliminar de Arbol 1: ");
            scanf("%d",&dato);
            if(eliminarNodo(ptrRef3, dato) == 1)
                printf("\nNodo no encontrado en Arbol 1");
            else
                printf("\nNodo %d eliminado de Arbol 1", dato);
            break;

        case 8:
            printf("\nNodo a eliminar de Arbol 2: ");
            scanf("%d",&dato);
            if(eliminarNodo(ptrRef4, dato) == 1)
                printf("\nNodo no encontrado en Arbol 2");
            else
                printf("\nNodo %d eliminado de Arbol 2", dato);
            break;
